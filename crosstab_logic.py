...
def compute_crosstab(
    ...
    if cfg.row_scale and scale_series is not None and detected_labels is not None:
        ...
        for rk in row_keys:
            counts = {"__total": int((row_mem[rk] & answered_row).sum())}
            pcts = {
                "__total": round(100.0 * counts["__total"] / base_total, 1) if base_total else None
            }
            success_counts: dict[str, int] = {}
            ...
            sig = make_sig_meta(
                success_total=counts["__total"],
                base_total_local=base_total,
                success_counts=success_counts,
                base_counts=bases,
            )

            count_rows.append({"kind": "cat", "key": rk, "label": row_label(rk), "cells": counts, "sig": sig})
            pct_rows.append({"kind": "cat", "key": rk, "label": row_label(rk), "cells": pcts, "sig": sig})

    return {
        "col_headers": [{"key": "__total", "label": "Total (%)"}] + [{"key": ck, "label": col_label(ck)} for ck in col_keys],
        "counts": count_rows,
        "percents": pct_rows,
        "meta": {
